...
alpha1=zeros;
cellnumber=49*49;
for i=1+(cyclefinal-1)*cellnumber*30:cellnumber*30+(cyclefinal-1)*cellnumber*30
    cellindex=mod(i,cellnumber)+1;
...
hold on
plot(xfit*100,oriorderm(cyclefinal,:),'o--',Color=[0.8,0.8,0.8],MarkerSize=6)

end

oriordermfinal=zeros(1,30);
oriorderstd=zeros(1,30);
for i=1:20
    for j=1:endt
...
%figure(3)
fill([xfit*100, flip(xfit*100)], [oriordermfinal+oriorderstd, flip(oriordermfinal-oriorderstd)], [0.8 0.8 0.8])
hold on
plot(xfit*100,oriordermfinal(1,:),'o-',MarkerSize=12)
